(function(){
  ...
  function patchTitles(){
    try{
      if (typeof TITLES !== 'undefined') {
        Object.keys(TITLE_FIX).forEach(function(k){ if (TITLES[k]) TITLES[k] = TITLE_FIX[k]; });
      }
    }catch(e){}
  }
  ...
  function replaceText(node){
    ...
    node.childNodes.forEach(function(child){
      if (child.nodeType === Node.TEXT_NODE) {
        ...
        FIXES.forEach(function(f){ t = t.split(f[0]).join(f[1]); });
        child.nodeValue = t;
  ...
  function addStyle(){
    ...
    st.id = 'admin500-status-style';
    st.textContent = '.admin500-calm-strip{margin:0 0 14px;border:1px solid #bee8d4;border-left:7px solid #0f7a4d;background:#f0fbf7;color:#0b4d34;border-radius:16px;padding:12px 14px;line-height:1.5;box-shadow:0 10px 24px rgba(7,27,53,.06)}.admin500-calm-strip b{color:#064e3b}.top-meta a:first-of-type{background:#fff8e6;border:1px solid #f6ca60;border-radius:10px;padding:8px 10px;color:#071b35!important}';
    document.head.appendChild(st);
  }

  function addStrip(){
    var app = document.getElementById('app');
    if (!app || app.querySelector('.admin500-calm-strip')) return;
    var div = document.createElement('div');
    div.className = 'admin500-calm-strip';
    div.innerHTML = '<b>Admin Profesional 500 aktif:</b> baca status dulu, cek dobel dipisahkan, angka final hanya setelah sumber dikunci. Produksi/cukai sudah membaca 21 Mei; pasar/piutang dibaca sesuai status validasi.';
    app.prepend(div);
  }

  function run(){ patchTitles(); addStyle(); replaceText(document.body); addStrip(); }
