(function(){
  const VERSION = 'status_bahan16_v1';

  function addHppKaryawanMenu(){
    const nav = document.getElementById('nav');
    if (nav && !document.querySelector('.nav-btn[data-key="status_bahan16_gate"]')) {
    ...
    if (mobile && !mobile.querySelector('option[value="status_bahan16_gate"]')) {
      const group = document.createElement('optgroup');
      group.label = 'Biaya & Produktivitas';
      group.innerHTML = '<option value="status_bahan16_gate">Status Bahan 16</option>' +
