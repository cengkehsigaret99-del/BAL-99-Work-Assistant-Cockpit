(function(){
  const VERSION = 'sop_bahan16_v1';

  function addHppKaryawanMenu(){
    const nav = document.getElementById('nav');
    if (nav && !document.querySelector('.nav-btn[data-key="sop_bahan16_gate"]')) {
    ...
    if (mobile && !mobile.querySelector('option[value="sop_bahan16_gate"]')) {
      const group = document.createElement('optgroup');
      group.label = 'Biaya & Produktivitas';
      group.innerHTML = '<option value="sop_bahan16_gate">SOP Bahan 16</option>' +
